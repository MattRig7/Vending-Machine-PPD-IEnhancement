...
void LinkedList::loadStockDataDLL(const char* filename) {
    std::ifstream stockFile(filename);
    if (!stockFile.is_open()) {
    std::cerr << "Error: could not open stock file." << std::endl;
    EXIT_FAILURE;
...
void LinkedList::loadCoinsDataDLL(const char* filename) {  
    std::ifstream coinsFile(filename);
    if (!coinsFile.is_open()) {
        std::cerr << "Error: could not open stock file." << std::endl;
        exit(EXIT_FAILURE);
...
void LinkedList::displayItemsDLL() {
    NodeDLL* current = headDLL;
    while (current != nullptr) {
        StockDLL* stock = current->data;
        std::cout << stock->id << "|" 
...
void LinkedList::addStockDLL(const std::string& id, const std::string& name, const std::string& desc, int dollars, int cents, int onHand) {
